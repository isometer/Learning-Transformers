...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, d_model, num_heads):
        ...
        # from the above line I'm assuming (we'll see) that the data is split over the the several heads so that each only needs to work with a subset of the input data?
    ...
    def scaled_dot_product_attention(self, Q, K, V, mask=None):
        ...
        # These scores represent the strength of attention from each query to each key.
        if mask is not None:
        # """_summary_
        ...
        attn_probs = torch.softmax(attn_scores, dim=-1) # softmax converts real-number scores into a probability distribution.

        output = torch.matmul(attn_probs, V)
        return output
    
    def split_heads(self, x):
        # I don't really understand this code, but essentially I think the Q, K, and V matrices
        # are conglomerates of the Q, K, and V individually that each head cares about.
        # So, this function splits them so it can run the attention head code for each head all at once,
        # and combine_heads merges the output back together. I think.
